...
import {filter, switchMap, debounceTime, interval, Observable, BehaviorSubject, tap} from 'rxjs';
...
@Component({
    ...
    template: `<button (click)="addProduct()">
            Add
        </button>
        <br/>
        <br/>
        <input type="text" class="input" [formControl]="searchTerm" placeholder="Search">
        <br/>
        <br/>
        <p>Fetched {{seconds}} seconds ago</p>
        <ag-grid-angular class="ag-theme-alpine"
                         [rowData]="products$ | async"
                         [gridOptions]="gridOptions"
                         [columnDefs]="columnDefs"
                         (rowDoubleClicked)="openProduct($event)"></ag-grid-angular>
...
export class ProductListComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.productService.getAll().subscribe(() => this.resetTime());
        this.searchTerm.valueChanges
            .pipe(
                filter(Boolean),
                switchMap((searchTerm) => this.productService.searchProduct(searchTerm)),
                tap(() => this.resetTime())
            ).subscribe();
        interval(1000).pipe(
            tap(() => this.seconds = this.getSeconds())
        )
            .subscribe()
    }
    ...
    addProduct() {
        this.bottomSheet
            .open<ProductDetailComponent, Partial<Product>, Partial<Product>>(ProductDetailComponent, {data: {}})
            .afterDismissed()
            .pipe(
                filter(Boolean),
